...
public class Black extends Bird {
    ...
    private float explosionRadius = 350f; // Explosion radius
    ...
    public void explode(Obstacle[] obstacles, pig[] pigs) {
        if (!hasExploded) {
            hasExploded = true;
            System.out.println("Black Bird Explosion at: " + getX() + ", " + getY());
            ...
            for (int i = 0; i < pigs.length; i++) {
                if (pigs[i] != null) {
                    ...
                    System.out.println("Pig " + i + " at (" + pigs[i].getX() + ", " + pigs[i].getY() +
                        ") - Distance: " + distance);

                    if (distance <= explosionRadius) {
                        System.out.println("Destroying Pig " + i + " within explosion radius");
                        pigs[i].markAsDead();
                    }
                }
            }
